...
export class ConsoleOutput {
    ...
    static printBlock(title, lines) {
        ...
        const truncatedTitle = String(title).length > maxBlockWidth 
        ...
        const maxLineLength = Math.max(...truncatedLines.map(line => line.length), truncatedTitle.length);
        const border = "─".repeat(Math.max(0, maxLineLength + 4));
        
        console.log(chalk.blue(`┌${border}┐`));
        console.log(chalk.blue(`│ ${chalk.bold(truncatedTitle)}${" ".repeat(Math.max(0, maxLineLength - truncatedTitle.length + 3))}│`));
        console.log(chalk.blue(`├${border}┤`));
