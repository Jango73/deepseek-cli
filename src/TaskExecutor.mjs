      ...
      try {
        ...
        for (const action of actions) {
          ...
          if (action.type === "agent") {
            if (
              cliInstance &&
              typeof cliInstance.launchAgentFromAI === "function"
            ) {
              executedSomething = true;
              await cliInstance.launchAgentFromAI(
                action.agentId,
                action.message,
              );
              ...
              ConsoleOutput.info(
                `❌ Agent delegation unsupported: ${action.agentId}`,
              );
            }
            continue;
          }
          ...
          if (
            cliInstance &&
            typeof cliInstance.handleSpecialCommand === "function"
          ) {
            const handled = await cliInstance.handleSpecialCommand(
              action.content,
            );
            if (handled) {
              executedSomething = true;
              lastSummaryPrompt =
                "Agent command handled. Continue with the next instruction.";
              continue;
            }
          }

          if (
            action.content.toLowerCase() === "pause" ||
            action.content.toLowerCase() === "exit"
          ) {
            shouldBreak = true;
            executedSomething = true;
            break;
