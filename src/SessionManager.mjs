...
export class SessionManager {
    ...
    try {
      if (fs.existsSync(this.sessionFile)) {
        ...
        this.currentSessionId =
          data.currentSessionId || this.generateSessionId();
        ...
        if (file.endsWith(".json")) {
          try {
            const data = JSON.parse(
              fs.readFileSync(`${this.archivesDirectory}/${file}`, "utf8"),
            );
            archives.push({
  ...
  async switchToArchive(sessionId) {
    ...
    ConsoleOutput.info(
      `🔄 Switched to archived session: ${archiveData.description}`,
    );
    this.showSessionStatus();
    return true;
  }
  ...
  showSessionStatus() {
    ...
    if (historyCount === 0) {
      ...
      if (lastUserMsg) {
        ConsoleOutput.info(
          `   Last action: "${this.truncateOutput(lastUserMsg.content, 1)}"`,
        );
      }
    }
  }
